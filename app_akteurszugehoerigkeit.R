...
nameinst_input <- textInput("nameinst", label = "Institution/Organisation (ORGA)")

gender_input <- radioButtons("gender", label = "Geschlecht (SEX)",
                             choices = c("Nichts ausgewählt" = "",
                                         "männlich" = 0,
                                         "weiblich" = 1,
                                         "anderes/nicht klar bestimmbar" = 99))

aktbez_input <- selectInput("aktbez", label = "Zugehörigkeit zu Gesellschaftsbereich (AREA)",
                             choices = c("Nichts ausgewählt" = "",
                                         "Wissenschaft" = 100,
                                         "Politik" = 200,
                                         ...
                                         "Interessenverbände 1 (Kollektivgüter)" = 51,
                                         "Interessenverbände 2 (Partialinteressen)" = 52,
                                         "Sonstige" = 900))

disc_input <- radioButtons("discipline", label = "Wissenschaftliche Disziplin (DISC)",
                           choices = c("Nichts ausgewählt" = "",
                                       ...
                                       "Kein:e Forscher:in" = 11,
                                       "Nicht zu erkennen" = 99))

aktort_input <- radioButtons("aktort", label = "Nationale Verortung (NAT)",
                             choices = c("Nichts ausgewählt" = "",
                                         "Deutschland" = 1,
                                         "Anderes EU-Land" = 2,
                                         "USA" = 3,
    ...
    tabPanel(
      ...
      tabsetPanel(
        ...
        tabPanel(
          ...
          fluidRow(column(8, includeMarkdown("source_codebuch/inhaltliche_variablen_1.md")))
        ),
        tabPanel(
          title = "Inhaltliche Kategorien 2 - AREA",
          fluidRow(column(8, includeMarkdown("source_codebuch/inhaltliche_variablen_2.md")))
        ),
        tabPanel(
          title = "Inhaltliche Kategorien 3 - DISC",
          fluidRow(column(8, includeMarkdown("source_codebuch/inhaltliche_variablen_3.md")))
        ),
        tabPanel(
          title = "Inhaltliche Kategorien 4 - NAT",
          fluidRow(column(8, includeMarkdown("source_codebuch/inhaltliche_variablen_4.md")))
        )
      )
    )
  )
...
## Für das Einlesen der Datensätze muss ein Root-Verzeichnis angegeben werden und die shinyFileChoose-Funktion ausgeführt werden.
  ...
  roots=c(wd='.')
  shinyFileChoose(input, "dataset", roots=roots, filetypes=c('', 'RDS', 'Rds'), defaultPath='', defaultRoot='wd')
...
## Dafür werden die alten Elemente für das Einlesen des Datensatzes entfernt. Zuletzt wird der Index des aktuellen Akteurs um eins erhöht.
  ...
  observeEvent(input$start_button, {
    if(length(full_dataset[!full_dataset$coded,][["coded"]]) == 0){
