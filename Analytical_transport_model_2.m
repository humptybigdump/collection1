...
v=7.5e-7;    % average transport velocity m/s -> come from tracer experiments (method of moments)
...
R=34;   % Retardation coefficient [-]
lambda=1/(67*86400); % degradation time scale 1/s -> come from the footprint data base
%lambda=0.15/86400;

Damm=(z_crit*R/v) / (1/lambda);
...
time(n)=t_trans;
for i=1:length(z)           
   C_sim(i,n)=1/(sqrt(4*pi*D*t_trans))...     
