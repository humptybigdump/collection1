...
!-----END OF COMMON VARIABLES
      ...
      INTEGER*4 I001,I,I1,II,IS,I2,NNPICK,DURCH,DUTOT,J
      CHARACTER*500 CH001
      CHARACTER*32 GNAME,GEM(EMAX)
      REAL*8 ARR(EMAX)
!-----
!----- double read if NPICK > 0
      IF (NPICK.GT.0) THEN
      NNPICK=NPICK
      DUTOT=2
      ELSE
...
!----
      IF (DURCH.EQ.2) THEN
      NPICK=NNPICK
!+++ not check for phases with COM LINES, must be picked
!+++ check for endmembers of solutions to choose
      I=1
      DO WHILE(I.LE.NPICK)
       DO IS=1,NSOL
        IF (VERGL(SOLNAM(IS),PICK(I))) THEN
         DO J=I+1,NPICK
          PICK(J-1)=PICK(J)
         END DO
         NPICK=NPICK-1
...
!-----
!******************************
      SUBROUTINE READGUESS2
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
      include 'files.cmn'
!-----END OF COMMON VARIABLES
      INTEGER*4 I,II,I001,IS,IG,ICP,NGEM,ARR(EMAX)
      CHARACTER*500 CH001
      CHARACTER*16 GNAME,GEM(EMAX),CH16
      REAL*8 XGEM,SUMME,F1,F2
!=====
      DO IS=1,NSOL
       NGUESS(IS)=0
       CALL LABLA(SOLNAM(IS),I001)
       OPEN (UNIT=40,FILE='ini_'//SOLNAM(IS)(1:I001), &
       STATUS='OLD',ERR=16)
    1  READ(UNIT=40,FMT='(A)',END=16) CH001
       CALL GELI(CH001,F1)
       CALL GELI(CH001,F2)
       NGEM=0
       DO I=1,EMAX
       ...
       END DO
   10  CONTINUE
       READ(UNIT=40,FMT='(A)',END=15) CH001
       DO I=1,EMAX
        ARR(I)=0
       END DO
       DO I=1,NGEM
        DO II=1,NEND(IS)
         I001=0
         IF (NAME(EM(IS,II)).EQ.GEM(I)) THEN
          ARR(I)=II
          I001=II
         END IF
        END DO
       ...
       END DO
!---
       NGUESS(IS)=NGUESS(IS)+1
       DO I=1,EMAX
        GUESS(IS,NGUESS(IS),I)=0.0D0
       END DO
       DO I=1,NGEM
        CALL GELI(CH001,XGEM)
        GUESS(IS,NGUESS(IS),ARR(I))=XGEM
       END DO
!---
      GOTO 1
   15 CONTINUE
...
!-----END OF COMMON VARIABLES
      INTEGER*4 I,II,I001,IS,IG,ICP,NGEM,ARR(EMAX)
      CHARACTER*500 CH001
      CHARACTER*16 GNAME,GEM(EMAX)
      REAL*8 XGEM,SUMME
!-----
      DO I=1,NSOL
       NGUESS(I)=0
      END DO
      OPEN (UNIT=40,FILE='inimini',STATUS='OLD',ERR=99)
      ICP=0
    1 READ(UNIT=40,FMT='(A)',END=15) CH001
      IF (CH001.EQ.'!'.OR.CH001.EQ.' ') GOTO 1
      IF (INDEX(CH001,'SOLUTION').NE.0) THEN
       ICP=1
       GOTO 1
      END IF
!*****
!---- expecting solution name etc.
      IF (ICP.EQ.1) THEN
!-- find solution IG
       CALL TAXI(CH001,GNAME)
       IG=0
       DO IS=1,NSOL
        IF (SOLNAM(IS).EQ.GNAME) THEN
         IG=IS
         GOTO 10
        END IF
       END DO
...
!-- make ARR(I)
       ...
       DO I=1,NEND(IG)
        DO II=1,NGEM
         IF (GEM(II).EQ.NAME(EM(IG,I))) THEN
          ARR(II)=I
!check that
         END IF
        END DO
      ...
      END IF
!*****
!---- expecting concentrations
      IF (ICP.EQ.2) THEN
       NGUESS(IG)=NGUESS(IG)+1
       SUMME=0.0D0
       DO I=1,NGEM
        CALL GELI(CH001,XGEM)
        IF (ARR(I).NE.0) THEN
         GUESS(IG,NGUESS(IG),ARR(I))=XGEM
         SUMME=SUMME+XGEM
        END IF
       END DO
       IF (DABS(SUMME-1.0D0).GT.1D-6) THEN
        NGUESS(IG)=NGUESS(IG)-1
       ELSE
        DO I=1,NGEM
         GUESS(IG,NGUESS(IG),I)=GUESS(IG,NGUESS(IG),I)/SUMME
        END DO
       END IF
       GOTO 1
      END IF
...
!-----
      ...
      GENUG(1)=.FALSE.
      NULL(NPHA+1)=.FALSE.
!---- check first for pick
      IF (NPICK.GT.0) THEN
      I001=0
      DO I=1,NPICK
      IF (VERGL(NAM,PICK(I))) I001=1
      END DO
      IF (I001.EQ.0) THEN
      GENUG(1)=.FALSE.
      REJECT='not picked'
...
!---- aqueous phases (HKF equation)
      ...
      GENUG(3)=.TRUE.
      END IF
!---- aqueous phases (Holland and Powell 1998, not used)
...
!******************************
      ...
      COMMON /SECINFO/ SECTIO,PROVID,AINFO1
!-----END OF COMMON VARIABLES
      INTEGER*4 I,IIND(MPOMAX),IND(MPOMAX),POL,IS,I001,K,IM
...
!******************************
      ...
      COMMON /SECINFO/ SECTIO,PROVID,AINFO1
!-----END OF COMMON VARIABLES
      INTEGER*4 I,IIND(MPOMAX),IND(MPOMAX),POL,IS,I001,K,I1,I2,IM
      REAL*8 WWCP,WWH,WWS,WWV,WWK
      LOGICAL*4 CODE
...
!-----END OF COMMON VARIABLES
      ...
      IC,NROW,NCOL,RANG,COLNR(250),ROWNR(250),PERMU(SITMAX)
      REAL*8 MAT(250,250),MINSO(SIELMAX), &
      MAXSO(SIELMAX),XXPERM(250,SIELMAX),SUM
      CHARACTER*32 NAMPERM(250),CH16,COLNAME(250)
!-----
...
! 3030 FORMAT (' permus: ',I3,4X,20(1X,I3))
      DO 610,I=1,NSIEL(IS)
...
!******************************
      ...
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
      REAL*8 MAT(250,250),COLSUM(250),WERT,FF,F1, &
      FFC(250),FFR(250)
...
!******************************
      ...
      FFC(250),FFR(250)
      INTEGER*4 NROW,NCOL,RANG,PIV,IR,IC,JX,JY,I,II, &
      COLNR(250),ROWNR(250),I001,CPIMAX
!----
      RANG=0
      DO 500,PIV=1,NROW
