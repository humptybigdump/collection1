...
void Camera::updateStereoMatrices()
{
	...
	for (int i = 1; i < 3; ++i)
	{
		m_viewMatrix[i]    = glm::lookAt(m_position[i], m_position[0] + m_viewDirection * m_focalDistance, m_up);
	...
	float focal_distance)
{
	...
	m_focalDistance = focal_distance;
	m_speed = 1.f;
	...
	m_pitch(0.f)
{
	...
	m_pitch = std::asin(m_viewDirection.y);
	m_yaw = std::atan2(-m_viewDirection.z, m_viewDirection.x) - static_cast<float>(M_PI)/2.f;
