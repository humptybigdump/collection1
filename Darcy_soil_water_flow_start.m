...
%-----------------------------------------------------------------------
% Select soil hydraulic parameter of soil of interest SIL 
thr=0.067; % residual water content of the soil
ths=0.41; % Porosity of the soil, water content at saturation
alpha=2.0; % Air entry value in 1/m
n_vg=1.41; % width ot the pore size distribution
ks= 0.45    /(100*3600);% saturated hydraulic conductivity
l_vg=0.5;
...
q_geo=zeros(dim-1,1); % soil water flux using the geometric mean of k_l
q_har=zeros(dim-1,1); % soil water flux using the harmonic mean of k_l
q_ari=zeros(dim-1,1); % soil water flux using the aritmetric mean of k_l
% define flux at upper and lower boundary of model domain 
qu=0; %Infiltrating water amount /rainfall
qb=0; % flux leaving the lower boundary 

% Calculate k_l and psi_l at nodes  
for i=1:dim
...
subplot(2,2,4);
plot(q_geo(1:dim-1)./theta_l(1:dim-1),z(1:dim-1),'rx','linewidth',2);
hold on;
plot(q_ari(1:dim-1)./theta_l(1:dim-1),z(1:dim-1),'bo','linewidth',2);
ylim([1.1*min(z) 0.9*max(z)]);
