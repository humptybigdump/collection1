...
!----
      ...
      CHARACTER*80 PIXFNAME,ISODIVCH,INFILE
      CHARACTER*8 CH8
!--
      call clearscreen
      progname='DOMINO'
...
!>
      ...
      .AND.XVAR(1:2).NE.'A('.AND.XVAR(1:4).NE.'LNA('.AND.XVAR.NE.'BIN' &
      .AND.XVAR(1:5).NE.'LOGA('.AND.XVAR.NE.'TER') THEN
      IF (MULTI) STOP
      CALL SHOUTI
      WRITE (6,2012)
...
!>
      ...
      .AND.YVAR(1:2).NE.'A('.AND.YVAR(1:4).NE.'LNA('.AND.YVAR.NE.'BIN' &
      .AND.YVAR(1:5).NE.'LOGA('.AND.YVAR.NE.'PT'.AND.YVAR.NE.'THG') &
      THEN
      IF (MULTI) STOP
      CALL SHOUTI
      WRITE (6,2022)
...
!-----jump here for pix (achtung bei pix war UNIT=job nicht geschlossen (korrigiert))
   88 CONTINUE
      IF (SIMU) THEN
      CALL LABLA(SCRFILE,I)
...
!-----
      ...
      IRF,(REANR(NPUN-IRF+II),II=1,IRF)
 1012 FORMAT (1X,2I6,1X,' Y =',F11.4,' ,',3X,I4,' REACTIONS:',500I5)
      ELSE
      WRITE (6,1014) NCTT,NCTTOT,XZU(1), &
      IRF,(REANR(NPUN-IRF+II),II=1,IRF)
      WRITE (out,1014) NCTT,NCTTOT,XZU(1), &
      IRF,(REANR(NPUN-IRF+II),II=1,IRF)
 1014 FORMAT (1X,2I6,1X,' X =',F11.4,' ,',3X,I4,' REACTIONS:',500I5)
      END IF
      NCTT=0
      CALL FLUSH(6)
      RETURN
...
!=====
      ...
      IF (ASS1(IR).EQ.-1.OR.ASS2(IR).EQ.-1) GOTO 999
!=====
...
!-----
 ...
 1020 FORMAT (' LABELED WITH ASSEMBLAGES')
      ...
      IF (ASS1(IR).EQ.-1.OR.ASS2(IR).EQ.-1) THEN
      TEXRE(1,IR)=' '
      TEXRE(2,IR)=' '
      END IF
      END IF
...
!
!**** LOCAL VARIABLES                               
      integer i, j, k, i001, ivar, erst(3), letzt(3), pn, I1, nrprop, &
