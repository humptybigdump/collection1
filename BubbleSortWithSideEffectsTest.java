...
public class BubbleSortWithSideEffectsTest
{
	...
	public void runPrintArray(){
	 ...
	 @Test
	public void runArraySortWithSideEffects(){
		boolean sortedEitherWithSideEffects = true;
		int[] newArray = g(10);
		try{
			BubbleSort.sortArray(newArray);
		}catch(Exception e){}
		
		for(int i = 0;(i<(newArray.length-1) && 
				sortedEitherWithSideEffects == true);i = i+1){
			if (newArray[i]>newArray[i+1]){
				sortedEitherWithSideEffects = false;
			}
		}
		assertTrue ("new random array was not sorted with side effects!",sortedEitherWithSideEffects);
	}
