% function to simulate soil water dynamics using simple Euler forward
...
%----------------Intialisation 
% Select soil hydraulic parameter of soil of interest from table above
...
dt=10; %time step
i_time=1; % counter for each time
% define simulation grid 
dz=0.05; % discretization widht
z=-[0.05:dz:1]'; % simulation domain
dim=length(z); % size of the domain

% Specify initial soil moisture profile 
ip=find(z >=-0.1); % find upper nodes
theta_high=0.4; % 
theta_low=0.2;
...
%dt= 0.0002*dz/max(abs(q_geo(i)));
% Update soil water content at inner nodes
for i=2:dim-1
    % flux based on the geometric mean 
