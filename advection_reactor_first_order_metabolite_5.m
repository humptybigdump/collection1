...
dx=0.05; % %grid size
x=[0:dx:25.];% reactor domain and grid meters
...
lambda2=1/86400; % reaction time scale of first order reaction
...
while time < tmax
   ...
   % solve working equation 
    for j=2:length(x)-1
        ...
        C2(j)=C2_alt(j)-dt*v/(x(j)-x(j-1))*(C2_alt(j)-C2_alt(j-1))+dt*C_alt(j-1)*lambda;
