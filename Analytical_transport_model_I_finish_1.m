...
% Works also for sand bed reactor!
clear all;
close all;

% set model parameters
D=1e-9 ;    % dispersion coefficient m^2/s -> come from tracer experiments (method of moments)
...
time(n)=t_trans;
for i=1:length(z)
   C_sim_cons(i,n)=1/sqrt(4*pi*D*t_trans).*exp(-(z(i)-v*t_trans)^2/(4*D*t_trans)); % correspondes to Cw*theta/m0
end
