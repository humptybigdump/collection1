...
# Deviations are hard to see! -> Plot differences instead
# for(i in 1:8){
#   assign(x = paste0("Fn",i), value = Fn(get(paste0("mu",i)),get(paste0("sd",i))))
#   plot(x = FnY, col = "red", main = paste0("F",i),xlim = c(-7,7))
#   plot(get(paste0("Fn",i)), col = "black", add = TRUE,xlim = c(-7,7))
#   if(i == 1) legend("topleft", legend = c("Obs", "Forc"),
#                     lwd = c(2,2), col = c("red", "black"), border = FALSE)
# }
