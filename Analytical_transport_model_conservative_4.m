...
R=7.2; % Bromoxynil (values from PDB data base, maximum KF,)
DT50=(8*86400); % DT 50 in days from PDB Data base
lambda=log(2)/DT50; 
Z_fix=1;   % select depth for plotting C(Z_fix,time)
dt= 1*86400;% timestep with in s
max_time= 300*86400; % maximum transport time s
dz=0.05; %grid  size m
...
C_sim_ret=zeros(length(z),floor(max_time/dt)); % transport of the conservative tracer allocate memory
...
for i=1:length(z)
   ...
   C_sim_ret(i,n)=1/(sqrt(4*pi*D/R*t_trans))...
       *exp(-(z(i)-v/R*t_trans)^2/(4*D/R*t_trans))*exp(-lambda*t_trans);
end

plot(C_sim_cons(:,n),-z,'r-','linewidth',2);
hold on;
plot(C_sim_ret(:,n),-z,'b-','linewidth',2);
hold off;
...
hold on;
h3=plot(time(:)/3600,C_sim_ret(idepth,:),'b-','linewidth',2);
