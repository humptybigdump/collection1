...
D=1.9e-7;    % dispersion coefficient m^2/s -> come from tracer experiments (method of moments)
v=1.6e-6;    % average transport velocity m/s -> come from tracer experiments (method of moments)
Z_fix=1;   % select depth for plotting C(Z_fix,time)
dt= 0.1*86400;% timestep with in s
max_time= 360*86400; % maximum transport time s

R=31;        % Retardation coefficient [-] -> come from the footprint data base
dz=0.05; %grid  size m
%lambda=1/(150*86400); % degradation time scale 1/s -> come from the footprint data base
lambda=0.15/86400;
maxz=5; %maximum depth m
z_crit=Z_fix; %critical depth

Damm=(z_crit*R/v)*lambda;
...
title(['Concentration profile at time ' num2str(t_trans/3600) ' h, Damm =' num2str(Damm)],'fontsize',14);
...
figure;
h1=plot(time(:)/3600,C_sim(idepth,:),'k-','linewidth',2);
hold on;
h2=plot(time(:)/3600,C_sim_cons(idepth,:),'r-','linewidth',2);
title(['Concentration in depth ' num2str(Z_fix) ' m as function of time, Damm =' num2str(Damm)],'fontsize',14);
xlabel(' time [h]','fontsize',14);
ylabel(' normalized Concentration [1/m^3],','fontsize',14);
legend([h1 h2],'conservative solute', 'non conservative solute'); 
set(gca,'fontsize',14,'linewidth',2);
