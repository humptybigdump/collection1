...
% Process loop
tmax=3*86400; % maximum simulation time
time=0.; % start time
...
while time < tmax
   % Define time step with Courant criterion, maxium computation time and start time 
    ...
    C(1)=C_in;% upstream boundary constant concentration 
    C(length(x))=C_alt(length(x)-1); %right boundary free outflow
   % solve working equation 
    for j=2:length(x)-1
        % numerical solution add finite difference form of the solution
        % here
        C(j)=C_alt(j)-v*dt*(C_alt(j)-C_alt(j-1))/(x(j)-x(j-1))-lambda*C_alt(j-1)*dt; 
